...
function GroupHeader({ step, title }: { step: string; title: string }) {
  return (
    <div className="flex items-center gap-2.5">
      <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-gold font-mono text-[11px] font-bold text-ink-deep">
        {step}
      </span>
...
export function CotizarForm({ whatsapp }: { whatsapp: string | null }) {
    ...
    return (
      <div className="flex flex-col items-start gap-4 rounded-2xl border border-gold/40 bg-gold/10 p-6 md:p-8">
        ...
        <p className="text-muted-foreground">
          Revisamos los datos y te contactamos a la brevedad con una propuesta
  ...
  return (
    <form
      action={formAction}
      className="flex flex-col gap-8 rounded-2xl border border-border bg-card p-6 shadow-card md:p-8"
    >
      {state.error ? (
      ...
      <div className="flex flex-col gap-5">
        <GroupHeader step="1" title="Sobre tu evento" />
